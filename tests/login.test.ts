...
async function tentar(email: string, ip: string): Promise<string | null> {
  try {
...
describe("Bloqueio por conta", () => {
  it("trava depois de erros seguidos e recusa até a senha certa", async () => {
    ...
    for (let i = 0; i < MAX_POR_CONTA; i += 1) {
      assert.equal(await tentar(alvo.email, "10.0.0.1"), null, `tentativa ${i + 1} liberada`);
      await registrarFalha(alvo.email, "10.0.0.1");
    }

    const mensagem = await tentar(alvo.email, "10.0.0.1");
  ...
  it("o bloqueio não vaza para outras contas", async () => {
    ...
    for (let i = 0; i < MAX_POR_CONTA; i += 1) await registrarFalha(alvo.email, "10.0.0.2");

    assert.ok(await tentar(alvo.email, "10.0.0.2"), "o alvo está bloqueado");
    assert.equal(await tentar(vizinho.email, "10.0.0.3"), null, "o vizinho passa normalmente");
  });
  ...
  it("acertar a senha zera o contador", async () => {
    ...
    await registrarFalha(alvo.email, "10.0.0.4");
    await registrarFalha(alvo.email, "10.0.0.4");
    await registrarSucesso(alvo.id, alvo.email, "10.0.0.4");
    ...
    await registrarFalha(alvo.email, "10.0.0.4");
    await registrarFalha(alvo.email, "10.0.0.4");
    assert.equal(await tentar(alvo.email, "10.0.0.4"), null);
  });

  it("o bloqueio expira sozinho", async () => {
    const alvo = await criarFuncionario();
    for (let i = 0; i < MAX_POR_CONTA; i += 1) await registrarFalha(alvo.email, "10.0.0.5");
    assert.ok(await tentar(alvo.email, "10.0.0.5"), "bloqueado agora");
    ...
    assert.equal(await tentar(alvo.email, "10.0.0.5"), null, "liberado após o prazo");
  });
  ...
  it("e-mail que não existe não revela nada nem quebra", async () => {
    ...
    assert.equal(await db.loginAttempt.count({ where: { email: inexistente } }), 1);
  });
});
...
describe("Bloqueio por origem", () => {
  it("barra quem tenta poucas senhas em muitas contas", async () => {
    ...
    for (let i = 0; i < MAX_POR_ORIGEM; i += 1) {
      ...
      assert.equal(await tentar(vitima.email, ip), null);
      await registrarFalha(vitima.email, ip);
    }

    const nova = await criarFuncionario();
    const mensagem = await tentar(nova.email, ip);
  ...
  it("outra origem não é afetada", async () => {
    ...
    assert.equal(await tentar(limpa.email, "198.51.100.4"), null);
  });
});
